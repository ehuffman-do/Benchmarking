...
def test_retry_fields_parse() -> None:
    spec = parse_spec(make_spec_doc(sweep={
        "threads": [4], "duration_s": 5, "retries": 3, "retry_wait_s": 120,
    }))
    assert spec.sweep.retries == 3
    assert spec.sweep.retry_wait_s == 120
